...
const UPDATE_WORK_DIR = process.env.STARTUNE_UPDATE_DIR || path.join(__dirname, 'updates');
const UPDATE_DOWNLOAD_DIR = process.env.STARTUNE_UPDATE_DOWNLOAD_DIR || path.join(UPDATE_WORK_DIR, 'downloads');
const UPDATE_PATCH_BACKUP_DIR = process.env.STARTUNE_PATCH_BACKUP_DIR || path.join(UPDATE_WORK_DIR, 'backups', 'patches');
const APP_USER_DATA_DIR = process.env.STARTUNE_USER_DATA_DIR
  || (process.env.APPDATA && path.join(process.env.APPDATA, 'StarTune'))
  || (process.platform === 'darwin' && process.env.HOME && path.join(process.env.HOME, 'Library', 'Application Support', 'StarTune'))
  || (process.env.HOME && path.join(process.env.HOME, '.startune'))
  || path.join(__dirname, 'userdata');
try { fs.mkdirSync(APP_USER_DATA_DIR, { recursive: true }); } catch (e) {}
const BEATMAP_CACHE_DIR = process.env.STARTUNE_BEAT_CACHE_DIR || path.join(APP_USER_DATA_DIR, 'beatmaps');
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.STARTUNE_VERSION || APP_PACKAGE.version || '0.9.11';
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.startune && pkg.startune.update) || {};
  const repoHint = process.env.STARTUNE_UPDATE_REPOSITORY
  ...
  const owner = process.env.STARTUNE_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.STARTUNE_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.STARTUNE_UPDATE_MANIFEST
      || process.env.STARTUNE_UPDATE_MANIFEST_URL
      || process.env.STARTUNE_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.STARTUNE_UPDATE_MIRRORS || process.env.STARTUNE_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `StarTune-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `StarTune-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('StarTune v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `StarTune/${APP_VERSION}` },
    });
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `StarTune-${latestVersion}-Setup.exe`;
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `StarTune/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('StarTune v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `StarTune-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `StarTune-${version || APP_VERSION}.exe`;
}
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.startune-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'startune-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'startune',
      productName: APP_PACKAGE.productName || 'StarTune',
      version: APP_VERSION,
      update: {
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log(' StarTune 沉浸式音乐播放器  →  http://localhost:' + PORT);
