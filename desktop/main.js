...
const APP_NAME = 'StarTune';
const APP_USER_MODEL_ID = 'com.startune.desktop';
const APP_ICON_ICO_PATH = path.join(__dirname, '..', 'build', 'icon.ico');
...
function resolveAppIcon() {
  ...
  if (fs.existsSync(APP_ICON_ICO_PATH)) return APP_ICON_ICO_PATH;
  if (fs.existsSync(APP_ICON_PNG)) return APP_ICON_PNG;
  return undefined;
}
const APP_ICON = resolveAppIcon();
const NETEASE_LOGIN_PARTITION = 'persist:startune-netease-login';
const NETEASE_LOGIN_URL = 'https://music.163.com/#/login';
const QQ_LOGIN_PARTITION = 'persist:startune-qqmusic-login';
...
const CHROMIUM_PERFORMANCE_SWITCHES = [
  ...
  ['use-angle', process.platform === 'darwin' ? 'metal' : 'd3d11'],
];
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('startune-global-hotkey', { action });
}

function unregisterStarTuneGlobalHotkeys() {
...
function configureStarTuneGlobalHotkeys(bindings = []) {
  unregisterStarTuneGlobalHotkeys();
  const results = [];
  const seen = new Set();
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.STARTUNE_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.STARTUNE_CREATE_DESKTOP_SHORTCUT === '1';
}
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'StarTune desktop music player',
      icon: fs.existsSync(APP_ICON_ICO_PATH) ? APP_ICON_ICO_PATH : target,
      iconIndex: 0,
...
public class StarTuneMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([StarTuneMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('startune-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('startune-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('startune-desktop-lyrics-state', desktopLyricsState);
}
...
if (-not ("StarTuneNativeWin" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public class StarTuneNativeWin {
...
$progman = [StarTuneNativeWin]::FindWindow("Progman", $null)
$result = [IntPtr]::Zero
[StarTuneNativeWin]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$result) | Out-Null
$script:workerw = [IntPtr]::Zero
$enum = [StarTuneNativeWin+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$param)
  $shell = [StarTuneNativeWin]::FindWindowEx($top, [IntPtr]::Zero, "SHELLDLL_DefView", $null)
  if ($shell -ne [IntPtr]::Zero) {
    $script:workerw = [StarTuneNativeWin]::FindWindowEx([IntPtr]::Zero, $top, "WorkerW", $null)
  }
  return $true
}
[StarTuneNativeWin]::EnumWindows($enum, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { $script:workerw = $progman }
$target = [IntPtr]::new([Int64]${hwnd})
[StarTuneNativeWin]::SetParent($target, $script:workerw) | Out-Null
[StarTuneNativeWin]::SetWindowPos($target, [IntPtr]::Zero, 0, 0, 0, 0, 0x0013) | Out-Null
`;
...
function sendWallpaperState() {
  if (!wallpaperWindow || wallpaperWindow.isDestroyed()) return;
  wallpaperWindow.webContents.send('startune-wallpaper-state', wallpaperState);
}
...
ipcMain.handle('startune-hotkeys-configure-global', (_event, bindings) => {
  return configureStarTuneGlobalHotkeys(bindings);
});

ipcMain.handle('startune-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'startune-export.json').replace(/[\\/:*?"<>|]+/g, '-');
    const result = await dialog.showSaveDialog(owner, {
      title: '导出 StarTune 存档',
...
ipcMain.handle('startune-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: '导入 StarTune 存档',
      properties: ['openFile'],
      filters: [{ name: 'JSON', extensions: ['json'] }],
    });
...
ipcMain.handle('startune-open-update-installer', async (_event, filePath) => {
  try {
...
ipcMain.handle('startune-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('startune-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('startune-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('startune-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('startune-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('startune-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('startune-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('startune-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
...
ipcMain.handle('startune-wallpaper-set-enabled', async (_event, enabled, payload) => {
  try {
...
ipcMain.handle('startune-wallpaper-update', async (_event, payload) => {
  try {
...
async function createWindow() {
  ...
  process.env.STARTUNE_UPDATE_DIR = getUpdateDownloadDir();
  try {
