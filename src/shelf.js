...
var shelfPeekStrip = 0;  // 0..1, 半收缩提示条的可见度（始终微亮）
...
function updateShelfHoverCueFromPointer(e) {
  ...
  if (shelfHoverCue.guide) {
    inZone = canShowShelfHoverCueAt(e);
  } else {
    // 非 guide 模式下直接判断热区条件（半收缩模式核心路径）
    inZone = !document.body.classList.contains('splash-active')
      && !visualGuideActive && !emptyHomeActive && !homeForcedOpen
      && shelfManager && shelfManager.getMode && shelfManager.getMode() === 'side'
      && !shelfPinnedOpen
      && !(shelfManager.hasOpenContent && shelfManager.hasOpenContent())
      && !isPointerOverUi(e)
      && (isShelfClickZone(e) || (shelfPreviewIsVisible() && isShelfPreviewUseZone(e)));
  }
...
function tickShelfHoverCue(dt) {
  ...
  // 鼠标离开热区后 400ms 开始收缩（比原来更快反馈）
  if (!shelfHoverCue.guide && !shelfHoverCue.zoneActive && performance.now() - shelfHoverCue.lastAt > 400) shelfHoverCue.target = 0;
  var target = shelfHoverCue.guide ? 1 : shelfHoverCue.target;
  // 展开用更快的插值率(0.18)，收缩用更慢的(0.085)形成弹性惰性感
  var rate = target > shelfHoverCue.value ? 0.18 : 0.085;
  ...
  // 半收缩提示条：shelf 有内容且处于 auto 隐藏模式时，始终保持微亮
  var wantPeek = (shelfManager && shelfManager.getMode && shelfManager.getMode() === 'side'
    && !shelfPinnedOpen && !shelfAlwaysVisible()
    && shelfManager.getCards && shelfManager.getCards().length > 0) ? 1 : 0;
  var peekRate = wantPeek > shelfPeekStrip ? 0.06 : 0.04;
  shelfPeekStrip += (wantPeek - shelfPeekStrip) * Math.min(1, peekRate * Math.max(1, dt * 60));
  if (shelfPeekStrip < 0.005 && !wantPeek) shelfPeekStrip = 0;
...
function makeShelfManager() {
  ...
  function placeCard(card, i, totalCards, modeIs) {
    ...
    if (modeIs === 'side') {
      ...
      // 半收缩偏移: shelfVisibility 低时向右退缩，高时回到正常位置
      var retractMix = shelfPinnedOpen || detailOpenSide || passiveAlways ? 0 : Math.max(0, 1 - shelfVisibility * 2.8);
      var retractX = retractMix * (layout.portrait ? 0.52 : 1.05);
      var retractRotY = retractMix * (layout.portrait ? 0.06 : 0.14);
      var px = layout.sideX + absD * layout.sideXStep - (detailOpenSide ? layout.sideDetailShift : 0) + entry * layout.sideEntryX + retractX;
      ...
      if (skullShelfPose && camera) {
        ...
        card.mesh.rotateY(layout.sideRotY + retractRotY + (1 - reveal) * 0.012 + parX * 0.006 * parWeight);
      } else {
        var safeRotY = wallpaperShelfPose ? 0.12 : layout.sideRotY;
        var safeEntryRotY = wallpaperShelfPose ? 0.05 : 0.16;
        card.mesh.rotation.y = (safeShelfPose ? safeRotY : layout.sideRotY) + retractRotY + (1 - reveal) * safeEntryRotY + parX * (safeShelfPose ? 0.014 : 0.038) * parWeight;
...
void main(){ vec4 t = texture2D(uDotTex, gl_PointCoord); if (t.a < 0.02) discard; gl_FragColor = vec4(vC, t.a * vA); }`,
  ...
  return {
    ...
    update: function(dt) {
      ...
      // v9: shelf 半收缩模式 — 平时露出微亮提示条，光标接近时快速弹出。
      var targetVis;
      if (!appRevealed) {
        targetVis = 0;
      } else if (mode === 'side') {
        ...
        else if (contentOpen || shelfPinnedOpen || shelfAlwaysVisible()) targetVis = 1.0;
        else {
          // 半收缩：始终保持 peekBase(0.06~0.09) 的微亮，hover 时快速升至全亮
          var peekBase = shelfPeekStrip * 0.09;
          targetVis = cueVis > 0.01 ? Math.max(peekBase + 0.10, cueVis * 0.92) : peekBase;
        }
      } else {
        targetVis = allItems.length ? 1.0 : 0;
      }
      // 展开更快(0.28)，收缩更慢(0.12) — 弹性手感
      var visRate = targetVis > shelfVisibility ? 0.28 : 0.12;
      shelfVisibility += (targetVis - shelfVisibility) * Math.min(1, visRate * Math.max(1, dt * 60) / 60);
