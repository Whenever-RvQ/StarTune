...
var USER_FX_ARCHIVE_STORE_KEY = 'startune-user-fx-archives-v1';
var USER_FX_ARCHIVE_EXPORT_TYPE = 'startune-user-fx-archive';
...
function safeArchiveFileName(name) {
  return String(name || 'StarTune 用户存档').replace(/[\\/:*?"<>|]+/g, '-').slice(0, 48) + '.json';
}
  ...
  modal.innerHTML =
    '<div class="hotkey-dialog" role="dialog" aria-modal="true" aria-label="热键设置">' +
      '<div class="hotkey-head">' +
        '<div><div class="hotkey-title">热键设置</div><div class="hotkey-sub">局内热键只在 StarTune 窗口内生效；全局热键会向系统注册，并检测是否被占用。</div></div>' +
...
function hotkeyStatusMarkup(scope, actionKey, binding, duplicate) {
  ...
  if (duplicate && duplicate[binding] > 1) return '<span class="hotkey-status conflict"><span class="source-icon">!</span>StarTune 内部重复</span>';
