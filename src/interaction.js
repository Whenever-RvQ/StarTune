...
function renderHomeDiscover() {
  ...
  if (weatherKicker) weatherKicker.textContent = 'StarTune · Your Library';
  if (sub) {
...
var SEARCH_HISTORY_STORE_KEY = 'startune-search-history';
...
async function attemptAudioPlay(opts) {
  ...
  try {
    ...
    if (opts.resumeAt && isFinite(opts.resumeAt) && opts.resumeAt > 0) {
      try { audio.currentTime = opts.resumeAt; } catch (e) {}
    }
...
async function togglePlay() {
  ...
  try {
    ...
    if ((!audio || !audio.src) && playQueue.length && currentIdx >= 0) {
      var _resumeOpts = { manual: true };
      if (typeof _pendingResumeAt !== 'undefined' && _pendingResumeAt > 0) { _resumeOpts.resumeAt = _pendingResumeAt; _pendingResumeAt = 0; }
      await playQueueAt(currentIdx, _resumeOpts);
      return;
    }
    if (!audio) return;
...
function cyclePlayMode() {
  ...
  try { localStorage.setItem(PLAYBACK_MODE_STORE_KEY, playMode); } catch (e) {}
...
function applyControlGlassChromaticOffset() {
  ...
  var filter = document.getElementById('startune-control-glass-filter');
  if (!filter) return;
...
function supportsControlGlassSvgFilter() {
  try {
    ...
    div.style.backdropFilter = 'url(#startune-control-glass-filter)';
...
function switchPlaylistTab(tab) {
  ...
  var queueToolbar = document.getElementById('queue-toolbar-sticky');
  if (queueToolbar) queueToolbar.style.display = tab === 'queue' ? '' : 'none';
...
function playlistPanelDetailHtml(pl, provider) {
  ...
  return '<div class="pl-inline-detail" data-pl-detail="' + escHtml(key) + '">' +
    '<div class="pl-detail-sticky">' +
      ...
      '<div class="pl-detail-actions"><button class="pl-detail-play" type="button" data-pl-detail-play="' + escHtml(key) + '"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>播放歌单</button><button class="fx-mini-btn ghost pl-detail-top-btn" type="button" data-pl-detail-top="1">回到顶部</button>' +
      '<div class="pl-detail-search-box" data-pl-detail-search="1"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg><input type="text" placeholder="搜索歌单内歌曲..." autocomplete="off" spellcheck="false" data-pl-detail-search-input="1"><span class="pl-detail-search-count" data-pl-detail-search-count="1"></span><button type="button" class="pl-search-clear" data-pl-detail-search-clear="1">&times;</button></div></div>' +
    '</div>' +
...
// ---------- 歌单内搜索 ----------
var plDetailSearchQuery = '';
var plDetailSearchTimer = null;
function filterPlaylistDetailBySearch(query) {
  query = String(query || '').trim().toLowerCase();
  plDetailSearchQuery = query;
  var detail = document.querySelector('.pl-inline-detail');
  if (!detail) return;
  var rows = detail.querySelectorAll('.pl-detail-row[data-pl-detail-row]');
  var tracks = playlistPanelDetailState.tracks || [];
  var matchCount = 0, firstMatchRow = null;
  for (var i = 0; i < rows.length; i++) {
    var row = rows[i], idx = parseInt(row.getAttribute('data-pl-detail-row'), 10), song = tracks[idx];
    if (!query) { row.classList.remove('search-hidden', 'search-highlight'); continue; }
    var name = ((song && song.name) || '').toLowerCase();
    var artist = ((song && song.artist) || '').toLowerCase();
    var album = ((song && song.album) || '').toLowerCase();
    var match = name.indexOf(query) >= 0 || artist.indexOf(query) >= 0 || album.indexOf(query) >= 0;
    row.classList.toggle('search-hidden', !match);
    row.classList.toggle('search-highlight', match);
    if (match) { matchCount++; if (!firstMatchRow) firstMatchRow = row; }
  }
  var countEl = detail.querySelector('[data-pl-detail-search-count]');
  if (countEl) countEl.textContent = query ? (matchCount + '/' + tracks.length) : '';
  var box = detail.querySelector('.pl-detail-search-box');
  if (box) box.classList.toggle('has-query', !!query);
  if (firstMatchRow && query) firstMatchRow.scrollIntoView({ behavior: 'smooth', block: 'center' });
  if (query && playlistPanelDetailState.tracks.length > (playlistPanelDetailState.renderLimit || 0)) {
    var panel = document.getElementById('playlist-panel');
    var keepTop = panel ? panel.scrollTop : 0;
    playlistPanelDetailState.renderLimit = playlistPanelDetailState.tracks.length;
    renderPlaylistPanelDetailState();
    if (panel) panel.scrollTop = keepTop;
    requestAnimationFrame(function(){ filterPlaylistDetailBySearch(query); });
  }
}
function clearPlaylistDetailSearch() {
  plDetailSearchQuery = '';
  var detail = document.querySelector('.pl-inline-detail');
  if (!detail) return;
  var input = detail.querySelector('[data-pl-detail-search-input]');
  if (input) input.value = '';
  filterPlaylistDetailBySearch('');
}
function handlePlaylistDetailSearchInput(e) {
  var value = e.target.value || '';
  if (plDetailSearchTimer) clearTimeout(plDetailSearchTimer);
  plDetailSearchTimer = setTimeout(function(){ filterPlaylistDetailBySearch(value); }, 150);
}
...
document.getElementById('pl-list').addEventListener('click', function(e){
  ...
  var searchClear = e.target && e.target.closest ? e.target.closest('[data-pl-detail-search-clear]') : null;
  if (searchClear) {
    e.preventDefault();
    e.stopPropagation();
    clearPlaylistDetailSearch();
    return;
  }
...
document.getElementById('pl-list').addEventListener('input', function(e){
  if (e.target && e.target.hasAttribute && e.target.hasAttribute('data-pl-detail-search-input')) {
    handlePlaylistDetailSearchInput(e);
  }
});
...
function bindPlaybackProgressEvents(audioEl) {
  if (!audioEl || audioEl._startuneProgressBound) return;
  audioEl._startuneProgressBound = true;
  ...
  var _progressSaveLastAt = 0;
  audioEl.addEventListener('timeupdate', function(){
    var now = Date.now();
    if (now - _progressSaveLastAt > 5000) {
      _progressSaveLastAt = now;
      schedulePlaybackProgressSave(300);
    }
  });
  audioEl.addEventListener('pause', function(){ savePlaybackProgress(); });
  audioEl.addEventListener('ended', function(){ savePlaybackProgress(); });
}
