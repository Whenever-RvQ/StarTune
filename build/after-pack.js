...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for StarTune icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'StarTune';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`StarTune executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`StarTune icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting StarTune resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'StarTune',
    '--set-version-string', 'ProductName', 'StarTune',
    '--set-version-string', 'CompanyName', 'StarTune',
